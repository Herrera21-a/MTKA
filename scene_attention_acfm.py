...
def main():
    ...
    acfm_1 = ACFM.acfm_module(128,4).to(opt.device)
    acfm_2 = ACFM.acfm_module(256,4).to(opt.device)
    acfm_3 = ACFM.acfm_module(512,4).to(opt.device)
    optimizer_a1 = optim.SGD(acfm_1.parameters(), lr=opt.lr, momentum=0.9, nesterov=opt.nesterov)
    optimizer_a2 = optim.SGD(acfm_2.parameters(), lr=opt.lr, momentum=0.9, nesterov=opt.nesterov)
    optimizer_a3 = optim.SGD(acfm_3.parameters(), lr=opt.lr, momentum=0.9, nesterov=opt.nesterov)
    ...
    for i in range(opt.n_epochs):
        ...
        for j in range(opt.iteration):
            ...
            f_1 = acfm_1(f_t1[0], f_t2[0], f_t3[0])
            f_2 = acfm_2(f_t1[1], f_t2[1], f_t3[1])
            f_3 = acfm_3(f_t1[2], f_t2[2], f_t3[2])
...
if __name__ == '__main__':
    ...
    dataset = 'scene3_attention_acfm' + time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
